...
from app.excel.template_generator import COMPANY_NAME
...
ROWS_PER_TABLE_PAGE = 20
FOOTER_TAGLINE = "Generated by DingTalk Attendance System"
...
def _resolve_company_display_name(company: Company) -> str:
    """Use corporate branding name for PDF headers."""
    if company.name and company.name not in {"Demo Company", "demo"}:
        return company.name
    return COMPANY_NAME
...
def _summary_cards(stats: AttendancePdfStats, styles: dict) -> Table:
    ...
    row_values = [Paragraph(f"<b>{value}</b>", styles["body"]) for _, value, _ in cards]
...
def _employee_table(rows: Sequence[AttendancePdfRow], styles: dict) -> Table:
    ...
    col_widths = [32 * mm, 48 * mm, 24 * mm, 20 * mm, 20 * mm, 20 * mm]
...
def _employee_tables_paginated(rows: Sequence[AttendancePdfRow], styles: dict) -> List:
    """Split the employee list into page-sized tables (max 20 rows per page)."""
    if not rows:
        return [Paragraph("暂无员工考勤数据。", styles["body"])]

    flow: List = []
    chunks = [rows[i : i + ROWS_PER_TABLE_PAGE] for i in range(0, len(rows), ROWS_PER_TABLE_PAGE)]
    for index, chunk in enumerate(chunks):
        if index > 0:
            flow.append(PageBreak())
            flow.append(Paragraph("员工考勤明细（续）", styles["section"]))
            flow.append(Spacer(1, 3 * mm))
        flow.append(_employee_table(chunk, styles))
    return flow
...
def _anomaly_section(rows: Sequence[AttendancePdfRow], styles: dict) -> List:
    anomalies = []
    for row in rows:
        if row.anomaly_summary:
            anomalies.append(row)
            continue
        if row.status == "warning" or row.absenteeism_count or row.lateness_count or row.missing_punch_count:
            anomalies.append(row)
    ...
    for row in anomalies:
        if row.anomaly_summary:
            summary = row.anomaly_summary
        else:
            parts = []
            if row.absenteeism_count:
                parts.append(f"旷工 {row.absenteeism_count}")
            if row.lateness_count:
                parts.append(f"迟到 {row.lateness_count}")
            if row.missing_punch_count:
                parts.append(f"缺卡 {row.missing_punch_count}")
            summary = "；".join(parts) if parts else "考勤状态异常"
...
def _make_page_footer(generated_at: datetime):
    ...
    def _draw_footer(canvas, doc) -> None:
        ...
        footer_y = 8 * mm
        canvas.drawString(14 * mm, footer_y, f"生成日期: {date_text}")
        canvas.drawCentredString(page_width / 2, footer_y, FOOTER_TAGLINE)
        canvas.drawRightString(page_width - 14 * mm, footer_y, f"Page {canvas.getPageNumber()}")
        canvas.restoreState()
...
def generate_attendance_pdf(
    ...
    *,
    ...
    styles = _styles()
    display_name = _resolve_company_display_name(company)
    ...
    doc = SimpleDocTemplate(
        ...
        author=generated_by or "DingTalk Attendance System",
    )

    page_footer = _make_page_footer(generated_at)

    story: List = []
    story.append(_header_table(display_name, year, month, styles))
    story.append(Spacer(1, 8 * mm))
    story.append(_summary_cards(stats, styles))
    story.append(Spacer(1, 6 * mm))
    story.append(Paragraph("员工考勤明细", styles["section"]))
    story.extend(_employee_tables_paginated(rows, styles))
    story.append(Spacer(1, 6 * mm))
