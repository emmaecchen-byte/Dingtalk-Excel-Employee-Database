...
Runs after Excel upload (manual_changes batch) and on DingTalk webhook field updates.
"""
...
def _conflict_list_item(conflict: Conflict, employee_name: str = "") -> Dict[str, Any]:
    return {
        "id": conflict.id,
...
def _latest_unmerged_manual_change(
    db: Session,
    *,
    company_id: int,
    year: int,
...
def detect_conflicts(
    ...
    *,
    ...
    """
    Compare manual Excel edits against current DingTalk-backed attendance values.

    For each manual change:
    1. Read the current value from ``monthly_attendance`` (DingTalk truth in DB)
    2. If ``last_sync_from_dingtalk`` is after the manual change and values differ,
       create a pending ``conflicts`` row
    3. Otherwise apply the manual value and mark ``manual_changes.merged_to_truth=True``
    """
    ...
    for manual_change in manual_changes_list:
        ...
        if dingtalk_updated_after_change and has_value_conflict:
            ...
            result.conflicts_list.append(
                _conflict_list_item(conflict, _employee_name(record))
            )
            continue
...
@dataclass
class WebhookConflictCheckResult:
...
def check_for_conflicts_on_update(
    db: Session,
    *,
    company_id: int,
    year: int,
    ...
    """
    Check for conflicts when a DingTalk webhook updates a single field.

    If an unmerged manual change exists and differs from the incoming DingTalk value,
    creates a pending conflict and leaves ``monthly_attendance`` unchanged.
    Otherwise applies the DingTalk value and updates ``last_sync_from_dingtalk``.
    """
    record = (
        db.query(MonthlyAttendance)
        .options(joinedload(MonthlyAttendance.employee))
    ...
    previous_value = get_field_value(record, field_name)
    now = sync_timestamp or datetime.utcnow()

    manual_change = _latest_unmerged_manual_change(
        db,
        company_id=company_id,
        year=year,
        month=month,
    ...
    if manual_change and manual_change.new_value is not None:
        manual_value = manual_change.new_value
        dingtalk_newer = dingtalk_is_newer_than_change(record, manual_change)
        has_conflict = values_conflict(
            dingtalk_value,
            manual_value,
            old_value=manual_change.old_value or previous_value,
        )

        if (dingtalk_newer or record.last_sync_from_dingtalk is None) and has_conflict:
            conflict = _create_conflict_record(
                db,
                company_id=company_id,
                year=year,
            ...
            logger.info(
                "Webhook conflict on update: employee_id=%s field=%s dingtalk=%s manual=%s",
                employee_id,
                field_name,
                dingtalk_value,
                manual_value,
            )
            return WebhookConflictCheckResult(
                conflict=conflict,
                applied=False,
                previous_value=previous_value,
            )

    apply_field_value(record, field_name, dingtalk_value)
